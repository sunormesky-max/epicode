...
static GLOBAL_LIBRARY: std::sync::OnceLock<std::sync::Arc<LibraryStore>> = std::sync::OnceLock::new();
...
impl LibraryStore {
    pub fn open(db_path: &std::path::Path, vector: Option<Arc<VectorLayer>>) -> Result<Self, String> {
        ...
        let rows: Vec<(i64, Option<Vec<u8>>)> = {
            match conn.prepare("SELECT id, embedding FROM library_chunks WHERE embedding IS NOT NULL") {
                Ok(mut s) => s
        ...
        Ok(Self { conn: Mutex::new(conn), hnsw: RwLock::new(restored), vector })
    }

    pub fn create_collection(&self, owner: &str, name: &str, visibility: &str, plan_gate: Option<&str>) -> Result<i64, String> {
    ...
    pub fn collection_owner(&self, collection_id: i64) -> Option<String> {
        self.conn
            .lock()
            .query_row("SELECT owner FROM library_collections WHERE id=?1", params![collection_id], |r| r.get(0))
            .ok()
    }
    ...
    pub fn ingest(
        ...
        let owner = self.collection_owner(collection_id).ok_or("collection not found")?;
        if owner != caller {
            return Err("only collection owner can ingest".into());
        }
        let vl = self.vector.as_ref().ok_or("embedding layer unavailable")?.clone();

        // 计划阶段: 确定item映射与待嵌chunk
        struct PlanChunk { item_id: i64, chunk_no: i64, content: String, hash: i64 }
            ...
            for it in items {
                let item_id = if let Some(r) = &it.client_ref {
                    match conn.query_row(
                        "SELECT id FROM library_items WHERE client_ref=?1", params![r], |x| x.get::<_, i64>(0),
                    ) {
                        Ok(id) => id,
                        Err(_) => {
                            conn.execute(
                ...
                for ch in &it.chunks {
                    if ch.content.trim().is_empty() { continue; }
                    let h = super::search_engine::hash_content(&ch.content) as i64;
                    let exists: bool = conn
                        .query_row("SELECT 1 FROM library_chunks WHERE content_hash=?1", params![h], |_| Ok(true))
                        .unwrap_or(false);
                    let exists_at_slot: bool = conn
                        .query_row(
                            "SELECT 1 FROM library_chunks WHERE item_id=?1 AND chunk_no=?2",
                            params![item_id, ch.chunk_no], |_| Ok(true),
                        )
                        .unwrap_or(false);
                    if exists || exists_at_slot {
                        skipped_plan += 1;
                    } else {
                        plan.push(PlanChunk { item_id, chunk_no: ch.chunk_no, content: ch.content.clone(), hash: h });
                    }
                }
            }
        }
        ...
        for (i, e) in vecs.into_iter().enumerate() {
            if e.len() == EMBEDDING_DIM { embedded.push((i, e)); }
        }
        ...
        if embedded.len() < plan.len() {
            ...
            for (i, _) in &embedded { got[*i] = true; }
            for i in 0..plan.len() {
                if !got[i] {
        ...
        };
        {
            ...
            for (i, emb) in &embedded {
                let p = &plan[*i];
                let cid: Option<i64> = self.conn.lock().query_row(
                    "SELECT id FROM library_chunks WHERE item_id=?1 AND chunk_no=?2", params![p.item_id, p.chunk_no], |r| r.get(0),
                ).ok();
                if let Some(id) = cid {
    ...
    pub fn search(&self, user_id: &str, plan_level: &str, query: &str, k: usize) -> Result<Vec<LibraryHit>, String> {
        ...
        let acl_cols: Vec<i64> = acl_stmt
            ...
            .unwrap_or_default();
        drop(acl_stmt);
        for c in &acl_cols { allowed.insert(*c); }
        for (id, vis, gate) in &cols {
            match vis.as_str() {
                "public" => { allowed.insert(*id); }
                "entitled" => {
        ...
        let owner_cols: Vec<i64> = {
            let mut s = conn.prepare("SELECT id FROM library_collections WHERE owner=?1").map_err(|e| e.to_string())?;
            s.query_map(params![user_id], |r| r.get(0))
                .map(|it| it.filter_map(|x| x.ok()).collect())
                .unwrap_or_default()
        };
        for c in &owner_cols { allowed.insert(*c); }
    ...
    pub fn create_request(&self, user_id: &str, title: &str, url: Option<&str>, note: Option<&str>) -> Result<i64, String> {
        let conn = self.conn.lock();
        conn.execute(
    ...
    pub fn list_requests(&self, status: Option<&str>) -> Vec<serde_json::Value> {
        ...
        match &status {
            Some(s) => stmt.query_map(params![s], map_row).map(|it| it.filter_map(|x| x.ok()).collect()).unwrap_or_default(),
            None => stmt.query_map([], map_row).map(|it| it.filter_map(|x| x.ok()).collect()).unwrap_or_default(),
        }
    }

    /// 处理请求: only_owner校验在handler层; accept时可选直接灌装一条(item入库)
    pub fn handle_request(&self, request_id: i64, action: &str, handler_note: Option<&str>) -> Result<(), String> {
        ...
        if n == 0 { return Err("request not found or already handled".into()); }
        Ok(())
    }

    pub fn pending_request_count(&self) -> i64 {
        self.conn.lock().query_row(
            "SELECT COUNT(*) FROM library_requests WHERE status='pending'", [], |r| r.get(0),
        ).unwrap_or(0)
    }

    pub fn set_collection_visibility(&self, collection_id: i64, visibility: &str) -> Result<(), String> {
        if !matches!(visibility, "private" | "entitled" | "public") {
            return Err("visibility must be private|entitled|public".into());
        }
        let conn = self.conn.lock();
        ...
        if n == 0 { return Err("collection not found".into()); }
        Ok(())
    }

    /// L1fix: 词面回退检索 — ONNX降级时不瘫
    fn search_keyword(&self, user_id: &str, plan_level: &str, query: &str, k: usize) -> Result<Vec<LibraryHit>, String> {
        let conn = self.conn.lock();
        let allowed = Self::allowed_collections(&conn, user_id, plan_level);
        let words: Vec<&str> = query.split_whitespace().filter(|w| w.chars().count() >= 2).collect();
        ...
        let rows: Vec<LibraryHit> = stmt.query_map(params![like, k * 3], |r| {
            Ok(LibraryHit {
                chunk_id: r.get(0)?, chunk_no: r.get(1)?, content: r.get(2)?,
                item_id: r.get(3)?, title: r.get(4)?, client_ref: r.get(5)?,
                source_meta: r.get(6)?, collection_id: r.get(7)?, score: 0.0,
            })
        }).map(|it| it.filter_map(|x| x.ok()).collect()).unwrap_or_default();
        drop(stmt);
        Ok(rows.into_iter().filter(|h| allowed.contains(&h.collection_id)).take(k).collect())
    }

    fn allowed_collections(conn: &Connection, user_id: &str, plan_level: &str) -> std::collections::HashSet<i64> {
        use std::collections::HashSet;
        let mut allowed: HashSet<i64> = HashSet::new();
        let cols: Vec<(i64, String, Option<String>)> = conn.prepare(
            "SELECT id, visibility, plan_gate FROM library_collections",
        ).and_then(|mut s| s.query_map([], |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)))
            .map(|it| it.filter_map(|x| x.ok()).collect())).unwrap_or_default();
        for (id, vis, gate) in &cols {
            match vis.as_str() {
                "public" => { allowed.insert(*id); }
                "entitled" => {
                    let g = gate.as_deref().unwrap_or("");
                    if g.is_empty() || plan_level == "enterprise" || plan_level == g { allowed.insert(*id); }
                }
                _ => {}
            }
        }
        if let Ok(mut s) = conn.prepare("SELECT collection_id FROM library_acls WHERE principal=?1") {
            if let Ok(it) = s.query_map(params![user_id], |r| r.get::<_, i64>(0)) {
                for id in it.filter_map(|x| x.ok()) { allowed.insert(id); }
            }
        }
    ...
    pub fn chunk_count(&self) -> i64 {
        self.conn.lock().query_row("SELECT COUNT(*) FROM library_chunks", [], |r| r.get(0)).unwrap_or(0)
    }
}
