...
impl Engine {
    ...
    fn build(data_path: std::path::PathBuf, shared_vector: Option<Arc<VectorLayer>>, user_id: Option<&str>) -> Self {
        ...
        match storage.load_drive_signals() {
            ...
            Err(e) => tracing::warn!("[{}] failed to load drive signals: {}", uid, e),
        }
    ...
    pub fn scavenge(&self) -> serde_json::Value {
        self.scheduler.api_scavenge()
    }
    ...
    pub fn start_full_arc(&self, tick_ms: u64) {
        self.gateway.rebuild_hnsw();
        self.space.reseed_ports();
    ...
    pub fn final_save(&self) {
        self.scheduler.save_drive_queue();
        let kg = self.scheduler.kg_handle();
