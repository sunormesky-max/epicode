...
const WEB_LOG: &str = "/var/log/nginx/epocode.cn.access.log";
...
const EPICODE_KEY: &str = "tm-3fc7bdde0b2647cb9651b639a319263f";
...
const HONEYPOT_PORTS: &[u16] = &[2222, 3306, 5432, 6379, 8080, 8443, 27017, 3389];

const WHITELIST: &[&str] = &["127.0.0.1", "::1", "0.0.0.0", "::"];

const MONITORED_FILES: &[&str] = &[
    "/etc/nginx/conf.d/epocode.cn.conf",
    "/etc/ssh/sshd_config",
    "/etc/systemd/system/epocode.service",
    "/opt/tetramem/epicode-cloud",
...
fn epicode_remember(content: &str, labels: &[&str]) {
    ...
    let tmp_path = "/tmp/epicode_guard_mem.json";
    ...
    let success = Command::new("curl")
        .args(&[
            ...
            "-H", &format!("X-API-Key: {}", EPICODE_KEY),
...
fn open_honeypot_ports() {
    ...
    log_msg(&format!("Honeypot ports opened: {:?}", HONEYPOT_PORTS));
}
                ...
                if let Some(local) = parts.get(4) {
                    if let Some(port_str) = local.rsplit(':').next() {
                        if let Ok(port) = port_str.parse::<u16>() {
                            let is_ok = EXPECTED_PORTS.contains(&port) || port == 9111 || HONEYPOT_PORTS.contains(&port);
                            if !is_ok {
