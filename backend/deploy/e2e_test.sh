#!/bin/bash
set -euo pipefail
# TetraMem E2E test — run locally on the server

BIN="${1:-/opt/tetramen/target/release/tetramem-mcp}"
DATA_DIR=$(mktemp -d)
INPUT_FILE="/opt/tetramem/deploy/e2e_test.jsonl"
