...
async fn main() {
    ...
    let cors_origin = std::env::var("TETRAMEM_CORS_ORIGIN")
        .unwrap_or_else(|_| "https://epicode.cn".into());
    ...
    let auth_middleware = |axum::extract::State(st): axum::extract::State<CloudState>,
        ...
        if path.starts_with("/health") || path == "/" || path == "/docs" || path == "/openapi.yaml" || path == "/admin/panel" || path == "/v1/login" || path == "/v1/skills/explore" || path == "/stats/public" || path == "/v1/agent-guide" {
        ...
        let user_info = match st.user_mgr.authenticate(api_key) {
            Some(u) => u,
            None => {
                tracing::warn!("auth failed for key length={} prefix={}", api_key.len(), &api_key.get(..4.min(api_key.len())).unwrap_or(""));
    ...
    let cors = CorsLayer::new()
        .allow_origin(cors_origin.parse::<axum::http::HeaderValue>().unwrap_or_else(|_| "https://epicode.cn".parse().unwrap()))
...
async fn user_stats(
    ...
    (StatusCode::OK, Json(serde_json::json!({
        ...
        "invite_code": if user.user_id == "sunorme" { st.user_mgr.current_invite_code() } else { String::new() },
