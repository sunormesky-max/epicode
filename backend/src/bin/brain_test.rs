...
fn main() {
    ...
    let energy = Arc::new(EnergyCenter::new(10000.0, 1.0, bus.sender(), bus.subscribe()));
    let knowledge = Arc::new(KnowledgeGraph::new());
    let cognitive = Arc::new(CognitiveEngine::new(&api_key, "deepseek-chat"));
    let classifier = Arc::new(epicode::engine::classifier::CategoryClassifier::new(&api_key, "deepseek-chat"));
    let embedding = Arc::new(epicode::engine::embedding::EmbeddingService::from_env());
    let vector = epicode::engine::vector::VectorLayer::load(std::path::Path::new("models")).ok().map(Arc::new);
    let gateway = Arc::new(GatewayCenter::new(
        space.clone(), energy.clone(), cognitive.clone(),
        classifier, bus.sender(), bus.subscribe(), knowledge.clone(),
        embedding, vector,
    ));

    let scheduler = Arc::new(SchedulerCenter::new(
        space.clone(), energy.clone(),
        knowledge.clone(), cognitive.clone(), gateway.clone(),
        bus.sender(), bus.subscribe(),
        1000, 10000.0,
    ));
    ...
    for (topic, texts) in &topics {
        for text in texts {
            match gateway.create_memory(text, vec![topic.to_string()]) {
                Ok(g) => { count += 1; println!("  [{}] {} → id={}", topic, &text[..30.min(text.len())], g.id); }
    ...
    println!("  状态摘要:");
    println!("    tick: {}, energy: {:.0}/{:.0}", state.tick, state.energy, state.max_energy);
    println!("    tetras: {}, vertices: {}, clusters: {}", state.total_tetras, state.total_vertices, state.clusters.len());
    for c in &state.clusters {
        let dominant = c.label_distribution.iter()
            .max_by_key(|(_, &v)| v)
            .map(|(k, v)| format!("{}:{}", k, v))
            .unwrap_or("none".into());
        println!("    簇{}: {}个 [{}] entropy={:.3} centroid=({:.1},{:.1},{:.1})",
    ...
    match cognitive.decide(&state) {
        Ok(response) => {
            ...
            for (i, action) in response.actions.iter().enumerate() {
                match action {
                    epicode::engine::cognitive::SchedulerAction::Pulse { origin, pulse_type, ttl } => {
                        println!("    {}. pulse(origin={}, type={}, ttl={})", i+1, origin, pulse_type, ttl);
                    }
                    epicode::engine::cognitive::SchedulerAction::Fission { cluster_index } => {
                        println!("    {}. fission(cluster={})", i+1, cluster_index);
                    }
                    epicode::engine::cognitive::SchedulerAction::Fuse { cluster_a, cluster_b } => {
                    ...
                    epicode::engine::cognitive::SchedulerAction::Consolidate { ids, keep, summary } => {
                        println!("    {}. consolidate({:?} -> keep #{}): {}", i+1, ids, keep, summary);
                    }
                    epicode::engine::cognitive::SchedulerAction::MarkJunk { ids, reason } => {
                        println!("    {}. mark_junk({:?}): {}", i+1, ids, reason);
                    }
                    epicode::engine::cognitive::SchedulerAction::Relabel { id, add_labels, remove_labels, reason } => {
                        println!("    {}. relabel(#{} +{:?} -{:?}): {}", i+1, id, add_labels, remove_labels, reason);
                    }
            ...
            println!("\n  执行后: {} 簇 → {} 簇", clusters.len(), clusters_after.len());
