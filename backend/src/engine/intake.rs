...
impl MemoryIntake {
    pub fn process(content: &str, labels: &mut Vec<String>) -> IntakeResult {
        ...
        if class == MemoryClass::Noise {
            tracing::info!("[Intake] REJECTED as noise: {:?}", truncate_str(content, 60));
            return IntakeResult {
        ...
        tracing::info!("[Intake] type={} importance={:.2} rationale={} tags={:?} content={:?}",
    ...
    fn classify(content: &str, labels: &[String]) -> MemoryClass {
        ...
        if short {
            let noise_words = ["ok", "好的", "好", "yes", "no", "nope", "test", "测试", "嗯", "哦",
                "thanks", "谢谢", "好嘞", "行", "可以", "done", "收到", "了解"];
            if noise_words.iter().any(|w| trimmed.eq_ignore_ascii_case(w) || lower == *w) {
                return MemoryClass::Noise;
            }
        }

        let label_set: std::collections::HashSet<&str> = labels.iter().map(|s| s.as_str()).collect();
        if label_set.contains("decision") { return MemoryClass::Decision; }
        if label_set.contains("bug") { return MemoryClass::Bugfix; }
        if label_set.contains("pattern") { return MemoryClass::Pattern; }
        if label_set.contains("session-summary") || label_set.contains("session") { return MemoryClass::Session; }
        if label_set.contains("finding") { return MemoryClass::Finding; }
        if label_set.contains("experiment") { return MemoryClass::Experiment; }
        if label_set.contains("security") { return MemoryClass::Security; }
        if label_set.contains("identity") { return MemoryClass::Identity; }
        if label_set.contains("research") { return MemoryClass::Research; }

        if lower.contains("[decision]") || lower.contains("决策") || lower.contains("chosen:")
            || lower.contains("chose") || lower.contains("选用") || lower.contains("改用")
            || lower.contains("选定") || lower.contains("we decided") || lower.contains("we chose")
        {
            return MemoryClass::Decision;
        }
        if lower.contains("[bug]") || lower.contains("symptoms:") || lower.contains("root_cause:")
            || lower.contains("fix:") || lower.contains("修复了") || lower.contains("resolved by")
            || lower.contains("workaround:") || lower.contains("hotfix:")
        {
            return MemoryClass::Bugfix;
        }
        if lower.contains("[pattern]") || lower.contains("convention:") || lower.contains("always use")
            || lower.contains("never use") || lower.contains("必须") || lower.contains("约定")
            || lower.contains("规范") || lower.contains("best practice")
        {
            return MemoryClass::Pattern;
        }
        if lower.contains("[session]") || lower.contains("accomplished:") || lower.contains("会话总结")
            || lower.contains("session summary")
        {
            return MemoryClass::Session;
        }
        if lower.contains("[finding]") || lower.contains("发现") || lower.contains("insight:")
            || lower.contains("turns out") || lower.contains("关键发现")
        {
        ...
        if lower.contains("security ban:") || lower.contains("security alert:")
            || lower.contains("cve-") || lower.contains("vulnerability")
        {
            return MemoryClass::Security;
        }
        if lower.contains("身份") || lower.contains("identity") || lower.contains("i am")
            || lower.contains("我是") || lower.contains("我的名字")
        {
            return MemoryClass::Identity;
        }
        if lower.contains("[research]") || lower.contains("arxiv") || lower.contains("论文")
            || lower.contains("前沿") || lower.contains("survey")
        {
    ...
    fn score_importance(content: &str, class: &MemoryClass) -> f64 {
        ...
        if lower.contains("critical") || lower.contains("不能") || lower.contains("never")
            || lower.contains("绝对不能") || lower.contains("必须") || lower.contains("致命")
            || lower.contains("crucial") || lower.contains("vital")
        {
            boost += 0.5;
        } else if lower.contains("重要") || lower.contains("important")
            || lower.contains("关键") || lower.contains("key")
        {
            boost += 0.3;
        }
        ...
        let has_code = lower.contains("```") || lower.contains("fn ")
            || lower.contains("pub fn") || lower.contains("impl ")
            || lower.contains("cargo ") || lower.contains("use ");
    ...
    fn extract_rationale(content: &str, memory_type: &str) -> Option<String> {
        ...
        if let Some(pos) = lower.find("rationale:") {
            ...
            let end = r.char_indices().take_while(|(i, _)| *i < 300).last()
                .map(|(i, c)| i + c.len_utf8()).unwrap_or(r.len().min(300));
        ...
        let causal_patterns: &[&str] = &[
            "因为", "因为 ", "由于", "由于 ", "because", "because ",
            "since ", "as ", "原因是", "reason:", "why:", "所以",
            "therefore", "in order to", "目的是",
        ];
    ...
    fn extract_tags(content: &str) -> Vec<String> {
        ...
        let tech_keywords: &[(&str, &str)] = &[
            ("rust", "rust"), ("cargo", "rust"), ("tokio", "async"),
            ("python", "python"), ("typescript", "typescript"), ("javascript", "javascript"),
            ("react", "frontend"), ("tailwind", "frontend"), ("css", "frontend"),
            ("nginx", "nginx"), ("docker", "docker"), ("kubernetes", "k8s"),
            ("sqlite", "database"), ("postgres", "database"), ("redis", "database"),
            ("ssh", "ssh"), ("https", "https"), ("certbot", "ssl"),
            ("git", "git"), ("github", "github"), ("ci/cd", "cicd"),
            ("wasm", "wasm"), ("onnx", "ml"), ("embedding", "ml"),
            ("xss", "security-vuln"), ("csrf", "security-vuln"), ("cve", "security-vuln"),
            ("curl", "debugging"), ("日志", "logging"), ("log::", "logging"),
    ...
    pub fn check_conflict(content: &str, similar: &[(u64, f64, f64, MemoryPayload)]) -> Vec<u64> {
        let negation_words = [
            "不是", "不能", "错误", "修正", "已修正", "fix", "fixes", "fixed",
            "wrong", "incorrect", "不再", "改为", "换为", "renamed", "should not",
            "deprecated", "废弃", "移除", "removed", "instead of", "替代",
        ];
        ...
        for (id, sim, _bm25, payload) in similar {
            if *sim > 0.75 {
                ...
                if topic_overlap > 0.4 {
                    tracing::info!("[Intake] conflict detected: new memory vs id={} (sim={:.2} overlap={:.2})",
                        id, sim, topic_overlap);
                    conflicts.push(*id);
                }
            }
        }
    ...
    fn topic_overlap(a: &str, b: &str) -> f64 {
        let lower_a = a.to_lowercase();
        let lower_b = b.to_lowercase();
        let keywords_a: Vec<&str> = lower_a.split(|c: char| !c.is_alphanumeric() && c != '-')
            .filter(|w| w.len() >= 2)
            .collect();
        if keywords_a.is_empty() {
            return 0.0;
        }
        let keywords_b: std::collections::HashSet<&str> = lower_b.split(|c: char| !c.is_alphanumeric() && c != '-')
            .filter(|w| w.len() >= 2)
            .collect();
        let common = keywords_a.iter().filter(|w| keywords_b.contains(**w)).count();
...
mod tests {
    ...
    fn decision_classification() {
        ...
        let result = MemoryIntake::process("Use PostgreSQL instead of SQLite because we need concurrent writes", &mut labels);
        assert!(!result.is_noise);
    ...
    fn bugfix_classification() {
        ...
        let result = MemoryIntake::process("Fixed memory leak in WebSocket handler. root_cause: connection pool never closed", &mut labels);
        assert!(!result.is_noise);
    ...
    fn rationale_extraction_because() {
        let mut labels = vec!["decision".to_string()];
        let result = MemoryIntake::process("We chose Rust because memory safety is critical for our use case", &mut labels);
        assert!(result.rationale.is_some());
    ...
    fn tag_extraction() {
        let mut labels = vec![];
        let result = MemoryIntake::process("Deploy new nginx config with HTTPS and SSL certbot on the server", &mut labels);
    ...
    fn importance_critical_boost() {
        ...
        let r1 = MemoryIntake::process("Critical: never use unwrap in production code", &mut labels1);
    ...
    fn text_similarity_basic() {
        ...
        let sim2 = MemoryIntake::text_similarity(
            "Deploy nginx config",
    ...
    fn general_content_passes() {
        let mut labels = vec![];
        let result = MemoryIntake::process("This is a general note about the project status and timeline for next quarter", &mut labels);
        assert!(!result.is_noise);
    ...
    fn conflict_detection() {
        let payload = MemoryPayload {
            ...
            content_hash: 0, labels: vec![], timestamp: 0, aliases: vec![],
            embedding: vec![], importance: 1.0, enforced: false,
            rationale: None, access_count: 0, memory_type: None,
        };
        ...
        assert!(!conflicts.is_empty(), "should detect conflict with negation + topic overlap");
    }
}
