...
impl SkillEngine {
        ...
        if let Some(data) = self.storage.get_meta("skills_data") {
            if let Ok(loaded) = serde_json::from_str::<Vec<Skill>>(&data) {
                let mut skills = self
                    .skills
                    .lock()
                    .expect("skills mutex poisoned during load");
                let mut next_id = self
                    .next_id
                    .lock()
                    .expect("next_id mutex poisoned during load");
                for mut s in loaded {
                    if s.id >= *next_id {
                        *next_id = s.id + 1;
                    }
    ...
    fn persist(&self) {
        let skills = self
            .skills
            .lock()
            .expect("skills mutex poisoned during persist");
    ...
    fn alloc_id(&self) -> u64 {
        let mut next_id = self
            .next_id
            .lock()
            .expect("next_id mutex poisoned during alloc_id");
        let id = *next_id;
        *next_id += 1;
        id
    }
    ...
    pub fn get(&self, id: u64) -> Option<Skill> {
        self.skills
            .lock()
            .expect("skills mutex poisoned in get")
            .get(&id)
            .cloned()
    }
    ...
    pub fn fork(&self, source: &Skill, new_owner: String) -> Skill {
        ...
        let mut skills = self
            .skills
            .lock()
            .expect("skills mutex poisoned in fork (insert)");
    ...
    pub fn insert_skill(&self, skill: Skill) -> Skill {
        let id = if skill.id
            >= *self
                .next_id
                .lock()
                .expect("next_id mutex poisoned in insert_skill (check)")
        {
            let mut next_id = self
                .next_id
                .lock()
                .expect("next_id mutex poisoned in insert_skill (update)");
            *next_id = skill.id + 1;
            skill.id
        } else {
            self.alloc_id()
        ...
        let mut skills = self
            .skills
            .lock()
            .expect("skills mutex poisoned in insert_skill");
    ...
    pub fn submit_for_review(&self, id: u64) -> Result<Skill, String> {
        let mut skills = self
            .skills
            .lock()
            .expect("skills mutex poisoned in submit_for_review");
        let skill = skills.get_mut(&id).ok_or("skill not found")?;
    ...
    pub fn approve_skill(&self, id: u64) -> Result<Skill, String> {
        let mut skills = self
            .skills
            .lock()
            .expect("skills mutex poisoned in approve_skill");
        let skill = skills.get_mut(&id).ok_or("skill not found")?;
    ...
    pub fn reject_skill(&self, id: u64, reason: &str) -> Result<Skill, String> {
        let mut skills = self
            .skills
            .lock()
            .expect("skills mutex poisoned in reject_skill");
        let skill = skills.get_mut(&id).ok_or("skill not found")?;
        if skill.review_status != ReviewStatus::PendingReview {
            return Err("skill is not pending review".to_string());
        }
    ...
    pub fn review_pending(&self) -> Vec<Skill> {
        let skills = self
            .skills
            .lock()
            .expect("skills mutex poisoned in review_pending");
        skills
            .values()
    ...
    pub fn link_memory(&self, skill_id: u64, memory_id: TetraId) -> Result<(), String> {
        let mut skills = self
            .skills
            .lock()
            .expect("skills mutex poisoned in link_memory");
    ...
    pub fn purge_non_system(&self) -> usize {
        let mut skills = self
            .skills
            .lock()
            .expect("skills mutex poisoned in purge_non_system");
    ...
    pub fn match_skills(&self, query: &str, owner: &str, limit: usize) -> Vec<Skill> {
        let skills = self
            .skills
            .lock()
            .expect("skills mutex poisoned in match_skills");
