# TetraMem SDK — TypeScript

Zero-dependency TypeScript SDK for the TetraMem Cloud API.
...
```ts
import { TetraMemClient, TetraMemAdmin } from "./tetramem";

const client = new TetraMemClient("your-api-key");
...
```ts
const admin = new TetraMemAdmin("your-admin-key");
...
```ts
import { TetraMemError } from "./tetramem";

try {
  await client.remember("hello");
} catch (err) {
  if (err instanceof TetraMemError) {
...
```ts
const client = new TetraMemClient("key", "http://localhost:9110");
```
