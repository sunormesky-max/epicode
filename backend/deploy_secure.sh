#!/bin/bash
# TetraMem Cloud — 一键安全部署
# 用法: ./deploy_secure.sh

set -euo pipefail

echo "=== TetraMem Cloud 安全部署 ==="
...
docker build -f Dockerfile.cloud -t tetramem-cloud:latest .
...
services:
  tetramem:
    image: tetramem-cloud:latest
    container_name: tetramem-cloud
  ...
  nginx:
    image: nginx:alpine
    container_name: tetramem-proxy
    restart: unless-stopped
    ports:
      - "443:443"
    volumes:
      - ./nginx/ssl:/etc/nginx/ssl:ro
      - ./nginx/tetramem.conf:/etc/nginx/conf.d/default.conf:ro
    depends_on:
      - tetramem
    security_opt:
      - no-new-privileges:true
